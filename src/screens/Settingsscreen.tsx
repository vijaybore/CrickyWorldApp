...
import {
  ...
  Modal, StyleSheet, ActivityIndicator, Alert, StatusBar, Platform} from 'react-native'
  ...
  return (
    <ScrollView contentContainerStyle={{ padding: 20, paddingBottom: 60 }} showsVerticalScrollIndicator={false}>
      ...
      <View style={{ backgroundColor: T.card, borderRadius: 12, padding: 20, marginBottom: 24, borderWidth: 1, borderColor: T.border }}>
        ...
        <Pressable android_ripple={{ color: "rgba(255,255,255,0.12)" }} onPress={() => { if (name.trim()) { dispatch({ type: 'CREATE_T', name: name.trim() }); setName('') } }}
      ...
      {state.tournaments.length > 0 && (
        <>
          ...
          {[...state.tournaments].reverse().map((t: any) => {
            ...
            return (
              <Pressable android_ripple={{ color: "rgba(255,255,255,0.12)" }} key={t.id} onPress={() => dispatch({ type: 'OPEN_T', id: t.id })}
  ...
  return (
    <ScrollView contentContainerStyle={{ padding: 16, paddingBottom: 60 }} showsVerticalScrollIndicator={false}>
      ...
      <View style={{ flexDirection: 'row', flexWrap: 'wrap', gap: 10, marginBottom: 20 }}>
        {FORMATS.map(f => (
          <Pressable android_ripple={{ color: "rgba(255,255,255,0.12)" }} key={f.key} onPress={() => setFmt(f.key)}
      ...
      <View style={{ backgroundColor: canGen ? T.accentDim : T.card, borderRadius: 12, padding: 16, borderWidth: 2, borderColor: canGen ? T.accent : T.border }}>
        ...
        <Pressable android_ripple={{ color: "rgba(255,255,255,0.12)" }} onPress={gen} disabled={!canGen} style={[S.btn, !canGen && { opacity: 0.4 }]}>
  ...
  return (
    <ScrollView contentContainerStyle={{ padding: 14, paddingBottom: 60 }} showsVerticalScrollIndicator={false}>
      ...
      <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', marginBottom: 14 }}>
        ...
        <Pressable android_ripple={{ color: "rgba(255,255,255,0.12)" }} onPress={() => dispatch({ type: 'SET_VIEW', view: 'setup' })}
      ...
      {all.length === 0 && (
        <View style={{ backgroundColor: T.card, borderRadius: 12, padding: 40, alignItems: 'center', borderWidth: 1, borderColor: T.border }}>
          ...
          <Pressable android_ripple={{ color: "rgba(255,255,255,0.12)" }} onPress={() => dispatch({ type: 'SET_VIEW', view: 'setup' })} style={[S.btnSmall, { marginTop: 8 }]}>
                  ...
                  <View style={{ flex: 1, padding: 12 }}>
                    ...
                    <View style={{ flexDirection: 'row', gap: 8 }}>
                      {f.status === 'completed' ? (
                        ...
                        <Pressable android_ripple={{ color: "rgba(255,255,255,0.12)" }} onPress={() => navigation.navigate('Scoring', { id: f.realMatchId })}
                        ...
                        <>
                          <Pressable android_ripple={{ color: "rgba(255,255,255,0.12)" }} onPress={() => dispatch({ type: 'START_MATCH', fid: f.id })}
                            style={S.btn}>
                            <Text style={S.btnTxt}>▶ Start</Text>
                          </Pressable>
                          <Pressable android_ripple={{ color: "rgba(255,255,255,0.12)" }} onPress={() => Alert.alert('Delete', 'Delete this fixture?', [{ text:'Cancel', style:'cancel' }, { text:'Delete', style:'destructive', onPress:()=>dispatch({ type:'DEL_FIX', tid:t.id, fid:f.id }) }])}
...
function MatchSetupView({ t, f, dispatch }: { t: any; f: any; dispatch: (a: any) => void }) {
  ...
  return (
    <ScrollView contentContainerStyle={{ padding: 16, paddingBottom: 60 }} keyboardShouldPersistTaps="handled" showsVerticalScrollIndicator={false}>
          ...
          <View key={String(lbl)} style={{ marginBottom: 14 }}>
            ...
            <View style={{ flexDirection: 'row', gap: 8 }}>
              {[f.team1, f.team2].map((team: string) => (
                <Pressable android_ripple={{ color: "rgba(255,255,255,0.12)" }} key={team} onPress={() => (setter as any)(team)}
      ...
      <Pressable android_ripple={{ color: "rgba(255,255,255,0.12)" }} onPress={handleStart} disabled={loading} style={[S.btn, loading && { opacity: 0.6 }]}>
  ...
  return (
    <View style={S.root}>
      ...
      {t && state.view !== 'home' && state.view !== 'matchSetup' && (
        <View style={S.tabBar}>
          {TABS.map(tab => (
            <Pressable android_ripple={{ color: "rgba(255,255,255,0.12)" }} key={tab.key} onPress={() => dispatch({ type: 'SET_VIEW', view: tab.key })}
